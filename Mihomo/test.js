...
function main(config) {
  ...
  // 3.5 构建功能策略组 — 增强版（手动 + 自动测速 + fallback智能切换）
...
// === 2️⃣ 自动测速组（每 60 秒更新延迟）===
const autoGroup = {
  ...groupBaseOption,
  name: '自动测速',
  type: 'url-test',
  interval: 60, // ✅ 每 1 分钟测试一次
  tolerance: 50,
  url: 'https://www.gstatic.com/generate_204',
  proxies: [...regionGroupNames, ...allLocalProxyNames],
  icon: 'https://raw.githubusercontent.com/Koolson/Qure/master/IconSet/Color/Speed.png',
}

// === 3️⃣ 核心默认策略（自动 failover + 自动恢复）===
const defaultNodeGroup = {
  ...
  interval: 60, // ✅ 1分钟健康检查
  url: 'https://www.gstatic.com/generate_204',
  proxies: ['手动节点', '自动测速', '直连'],
  icon: 'https://raw.githubusercontent.com/Koolson/Qure/master/IconSet/Color/Proxy.png',
}

functionalGroups.push(manualGroup)
functionalGroups.push(autoGroup)
